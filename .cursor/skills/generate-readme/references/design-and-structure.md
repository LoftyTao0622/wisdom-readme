...
Badges are metadata. Add them when they answer a real adoption question: package version, supported runtime, build health, license, community entry point, or a concise technology-stack summary. When a technical project exposes at least two primary stack facts, include a compact row beneath the hero by default; omit it only for an explicitly plain style, an incompatible host, or a duplicate verified metadata row. Confirm each primary framework, runtime, or infrastructure component in a manifest, configuration, CI/deployment file, or maintained project document; prefer direct dependencies and declared runtimes, and omit transitive or incidental packages. Verify each destination and status source, use one badge style, and keep the group visually quiet and bounded rather than exhaustive. An evidence-backed stack row is metadata, not a generic badge wall.
