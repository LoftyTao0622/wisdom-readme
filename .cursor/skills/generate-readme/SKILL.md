...
For technical projects, include a compact technology-stack badge row by default when at least two primary stack facts help readers assess setup or integration. Omit it only when the user requests a plain style, the target host cannot render badges, or the row would duplicate an existing verified metadata row. Each item must be directly supported by the appropriate repository evidence: primary dependencies from manifests, resolved versions only for those declared dependencies (with lockfiles as corroboration), runtime requirements from runtime declarations, and license or build status from their authoritative files or services. Treat the row as one quiet metadata group, include only the primary technologies that help readers assess setup or integration, and omit transitive, incidental, or unverified items. This evidence-backed row is distinct from an exhaustive or decorative badge wall.
